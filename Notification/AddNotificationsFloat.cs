...
namespace Notification
{
    public partial class AddNotifications<T> where T : Notifiable
    {
        ...
        public AddNotifications<T> IfLowerThan(Expression<Func<T, float>> selector, 
                                               float min, string message = "")
        {
        ...
        public AddNotifications<T> IfGreaterThan(Expression<Func<T, float>> selector, float max, 
                                                 string message = "")
        {
            var val = selector.Compile().Invoke(_notifiableObject);
            var name = ((MemberExpression)selector.Body).Member.Name;
        ...
        public AddNotifications<T> IfGreaterOrEqualsThan(Expression<Func<T, float>> selector, 
                                                         float number, string message = "")
        {
            var val = selector.Compile().Invoke(_notifiableObject);
            var name = ((MemberExpression)selector.Body).Member.Name;
        ...
        public AddNotifications<T> IfLowerOrEqualsThan(Expression<Func<T, float>> selector, 
                                                       float number, string message = "")
        {
            var val = selector.Compile().Invoke(_notifiableObject);
            var name = ((MemberExpression)selector.Body).Member.Name;
        ...
        public AddNotifications<T> IfNotRange(Expression<Func<T, float>> selector, 
                                              float a, float b, string message = "")
        {
            var val = selector.Compile().Invoke(_notifiableObject);
            var name = ((MemberExpression)selector.Body).Member.Name;
        ...
        public AddNotifications<T> IfRange(Expression<Func<T, float>> selector, float a, float b, 
                                           string message = "")
        {
            var val = selector.Compile().Invoke(_notifiableObject);
            var name = ((MemberExpression)selector.Body).Member.Name;
        ...
        public AddNotifications<T> IfNotAreEquals(Expression<Func<T, float>> selector, float value, 
                                                  string message = "")
        {
            var val = selector.Compile().Invoke(_notifiableObject);
            var name = ((MemberExpression)selector.Body).Member.Name;
        ...
        public AddNotifications<T> IfAreEquals(Expression<Func<T, float>> selector, 
        ...
        /// <param name="message">Mensagem de erro (Opcional)</param>
        /// <returns>Dado um int, adicione uma notificação se for maior ou igual ao parametro passado</returns>
        public AddNotifications<T> IfGreaterOrEqualsThan(float val, float number, 
        ...
        /// <param name="message">Mensagem de erro (Opcional)</param>
        /// <returns>Dado um int, adicione uma notificação se for menor ou igual ao parametro passado</returns>
        public AddNotifications<T> IfLowerOrEqualsThan(float val, float number, string objectName, 
                                                       string message = "")
        {
            if (val <= number)
                _notifiableObject.AddNotification(objectName, string.IsNullOrEmpty(message) ? 
        ...
        /// <param name="message">Mensagem de erro (Opcional)</param>
        /// <returns>Dado um int, adicione uma notificação se não estiver entre alguns dois valores</returns>
        public AddNotifications<T> IfNotRange(float val, float a, float b, string objectName, 
                                              string message = "")
        {
            if (val < a || val > b)
                _notifiableObject.AddNotification(objectName, string.IsNullOrEmpty(message) ? 
        ...
        /// <param name="message">Mensagem de erro (Opcional)</param>
        /// <returns>Dada uma string, adicione uma notificação se não for igual a</returns>
        public AddNotifications<T> IfNotAreEquals(float val, float value, string objectName, 
                                                  string message = "")
        {
            if (val != value)
                _notifiableObject.AddNotification(objectName, string.IsNullOrEmpty(message) ? 
